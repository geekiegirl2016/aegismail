...
import { GMAIL_IMAP, OUTLOOK_IMAP } from '@aegismail/providers/imap';
...
import {
  buildAuthorizationUrl as buildMsAuthUrl,
  exchangeCodeForTokens as exchangeMsCode,
  parseIdToken as parseMsIdToken,
...
export async function registerOAuthRoutes(
  ...
  app.get('/v1/oauth/providers', async () => {
    ...
    const microsoft = microsoftOAuthFromConfig(deps.config);
    return {
      providers: [
        { id: 'google', configured: google !== null },
        { id: 'microsoft', configured: microsoft !== null },
      ],
    };
  });
  ...
  app.post('/v1/oauth/google/start', async (req, reply) => {
    try {
      ...
      const authUrl = buildGoogleAuthUrl(google, {
      ...
      const flowPromise = (async () => {
        ...
        const tokens = await exchangeGoogleCode(google, {
          code,
  ...
  app.post('/v1/oauth/microsoft/start', async (req, reply) => {
    try {
      const ms = microsoftOAuthFromConfig(deps.config);
      if (!ms) {
        throw new AegisError('provider_error', 'Microsoft OAuth client not configured.');
      }
      const body = StartBody.parse(req.body);
      ...
      const authUrl = buildMsAuthUrl(ms, {
        redirectUri: loopback.redirectUri,
        state,
        codeChallenge: pkce.challenge,
        ...(body.loginHint ? { loginHint: body.loginHint } : {}),
      ...
      const flowPromise = (async () => {
        const { code } = await loopback.result;
        const { tokens, idToken } = await exchangeMsCode(ms, {
          code,
          codeVerifier: pkce.verifier,
          redirectUri: loopback.redirectUri,
        });
        if (!idToken) {
          throw new AegisError(
            'provider_error',
            'Microsoft did not return an id_token — cannot determine user email.',
          );
        }
        const info = parseMsIdToken(idToken);

        const existing = accounts
          .list()
          .find((a) => a.provider === 'outlook' && a.emailAddress === info.email);
        ...
        if (existing) {
          ...
          const created = accounts.create({
            ...
            displayName: body.displayName,
            emailAddress: info.email,
            config: { ...OUTLOOK_IMAP, username: info.email },
          });
          accountId = created.id;
        }
